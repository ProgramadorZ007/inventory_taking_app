 ...
 * - WorkManager con ExistingWorkPolicy.KEEP evita que se encolen múltiples Workers.
 * - SincronizarPendientesUseCase itera sobre cada registro PENDIENTE individualmente:
 *   si el envío es exitoso, lo elimina de la BD local (no puede reenviarse).
 * - El servidor debe ser idempotente: si por alguna razón un registro llega dos veces,
 *   debería identificarlo por su UUID (idInventario) y no crear duplicados.
 */
